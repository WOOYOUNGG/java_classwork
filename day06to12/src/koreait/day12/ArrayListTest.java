...
//java.util.ArrayList 클래스를 사용해서 배열보다 편하게 데이터를 저장/접근합니다. -자료구조, 알고리즘
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> intList = new ArrayList<>();		//나중에 변수의 참조타입을 변경합니다.
										//1) 크기는 데이터를 추가할 때마다 증가됩니다. (데이터 삭제할 때는 크기 감소)
										//*기본형데이터 Integer : 래퍼클래스를 사용, <> 는 제너릭타입을 지정하는 기호.
		...
		System.out.println("list의 크기 :"+intList.size());		//size: 여기서는 5
		
		intList.add(2,99);	//데이터는 2번 인덱스 위치에 추가
		System.out.println(intList);
		System.out.println("list의 크기 :"+intList.size());		//size: 여기서는 6, 2번 인덱스에 99가 추가 됐으므로
		
		//삭제
		intList.remove(3);		//인덱스 3 위치 삭제
		System.out.println(intList);
		System.out.println("list의 크기 :"+intList.size());		//size: 여기서는 5, 3번 인덱스 7이 삭제 됐으므로
		...
		//Quiz	//intList 의 모든값을 index= .. element= .. 형식으로 출력해보세요 -> for문 (반복횟수는 데이터 개수 만큼)
