...
public class IntArrayTest {	//정수값 배열을 연습합니다.
	//배열 : 일반변수는 변수 1개에 데이터 1개 저장합니다. 많은 데이터를 저장하려면 변수를 많이 선언하지 않고 배열을 사용.
	...
	public static void main(String[] args) {
		...
		//배열 데이터의 위치(숫자 0부터 시작) : 인덱스, 기호[]
		System.out.println("arr1 배열 3번 째 데이터의 값 : " +arr1[2] ); //값? 89
		System.out.println("arr2 배열 3번 째 데이터의 값 : " +arr2[2] ); //값? 0
		...
		arr2[4]=78;
		System.out.println("arr2 배열 5번 째 데이터의 값 : " +arr2[4]);
		
		//1)Random 클래스로 1~46 범위의 값을 난수로 만들어서 arr2 배열에 10개 저장하기.
		Random r1 = new Random();
		
		for(int i=0;i<arr2.length;i++) {	//i<arr1.length는 배열이 크기가 변경되어도 잘 실행될 수 있습니다.
