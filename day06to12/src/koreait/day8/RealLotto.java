...
public class RealLotto {
	...
	public static void main(String[] args) {
		...
		System.out.println("--------------------------------------");
		int[] lotto=new int[6];
		int k;							//삭제할 위치의 인덱스 -> 랜덤한 겂으로 합니다.
		for(int cnt=0;cnt<6;cnt++) {
			
			//1.배열에서 데이터 삭제
			k=r.nextInt(45-cnt);			//경계값을 45(0~44),44(0~43),43(0~42),42(0~41),41(0~40),40(0~39)
			System.out.println("k="+k+", number ="+ numbers[k]); 		//몇 번 째 인덱스일까요?
		...
		//오름차순 정렬 : 작은 값에서 큰 값 순서대로 위치를 바꿉니다 (위의 코딩은 뽑은 숫자 순서대로 나열)
		Arrays.parallelSort(lotto);		//lotto 배열 값 순서대로 위치 변경합니다. (sort:정렬)
		
		System.out.print("정렬된 최종 선택 숫자 : ");
