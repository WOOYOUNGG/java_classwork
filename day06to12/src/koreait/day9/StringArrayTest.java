...
//String 클래스의 메소드 실행: charAt(), subString(2,5)을 각 배열 요소 문자열로 실행합니다.
		for(int i=0;i<strArray.length;i++) {
		...
		String[] names = new String[5];			//지정된 문자열이 없으므로 같은 null(없다) 상태
		for(int i=0;i<names.length;i++) 
			System.out.println("i="+i+",이름="+names[i]);	
		//String은 클래스, 문자열은 객체 -> names 배열, names[i] 둘 다 참조(주소)값을 저장합니다.
	
//			System.out.println("charAt(0): "+names[0].charAt(0) +
//				", subString(2,5): "+names[0].substring(2,5));	//문자열이 null 일 때 charAt 등 메소드 실행은 오류 발생.
